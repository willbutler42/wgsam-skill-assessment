...
cod <-
  gadgetstock('cod',gd$dir,missingOkay = TRUE) %>%
  gadget_update('stock',
                minage = 0,
                maxage = 20,
                minlength = 8,
  ...
  gadget_update('initialconditions',
                ...
                ##                          area = 1,
                ##                          age.factor = parse(text=sprintf(paste0('exp(-1*(cod.M+cod.init.F)*%1$s)*cod.init.%1$s'), age)) %>% 
                ##                            map(to.gadget.formulae) %>% 
                ##                            unlist(),   
                ##                          area.factor = '#cod.init.scalar',
                ##                          ## mean = von_b_formula(age,linf='cod.Linf',k='cod.k',recl='cod.recl'),
                ##                          mean = parse(text=sprintf('cod.Linf*(1-exp(-1*(0.01*cod.k)*(%1$s-(0.5+log(1-cod.recl/cod.Linf)/(0.01* cod.k)))))',age)) %>% # notice [... -(0.5+log ...] so length scaled considering that rec is in timestep 3
                ##                              map(to.gadget.formulae) %>% 
                ##                              unlist(),   
  ...
  gadget_update('doesrenew',
                normalparam = data_frame(year = year_range,
                                         ...
                                         area = 1,
                                         age = .[[1]]$minage,
                                         number = parse(text=sprintf('cod.rec.scalar*cod.rec.%s',year)) %>% 
                                           map(to.gadget.formulae) %>% 
                                           unlist(),
                                         ## mean = von_b_formula(age,linf='cod.Linf',k='cod.k',recl='cod.recl'),
                                         mean = parse(text=sprintf('cod.Linf*(1-exp(-1*(0.01*cod.k)*(%1$s-(0+log(1-cod.recl/cod.Linf)/(0.01* cod.k)))))',age)) %>% # notice [... -(0+log ...] so recl is mean length at recr time
                                             map(to.gadget.formulae) %>% 
                                             unlist(),   
                                         stddev = paste0('#',stock_names,'.rec.sd'),
                                         alpha = paste0('(* 1e-3 #',stock_names,'.walpha)'),
                                         beta = paste0('#',stock_names,'.wbeta')))
cod$initialconditions$minage <- 1
## cod$initialconditions$maxage <- 20
## cod$initialconditions$minlength <- 8.5
## cod$initialconditions$maxlength <- 17.5
