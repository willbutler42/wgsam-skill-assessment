...
defaults <- list(
    ...
    species = 'COD',
    length=mfdb_interval("len", seq(20, 140, by = 1)),
...
source('utils.R')
source('setup_fleets_cod.R')
source('setup_model_cod.R')
source('setup_catchdistribution_cod.R')
## source('setup_catchstatistic_cod.R')
source('setup_indices_cod.R')
source('setup_likelihood_cod.R')


Sys.setenv(GADGET_WORKING_DIR=normalizePath(gd$dir))
callGadget(s=1,log = 'init.log') #ignore.stderr = FALSE,

## update the input parameters with some initial guesses
read.gadget.parameters(sprintf('%s/params.out',gd$dir)) %>% 
  ...
  init_guess(paste0(stock_names,'.Linf'), grw.constants[1], 90, 150,0) %>%
  ...
  ## init_guess('cod.init.F',0.3,0.1,1,0) %>%
